  ...
  module Header
    def measure_header(node, opts)
      ...
      measure_block_padding_height(opts) +
        @pdf.height_of_formatted(data_node_children(node, header_opts), filter_block_hash(header_opts))
    end
    ...
    def draw_header(node, opts)
      ...
      with_block_padding(header_opts) do
        @pdf.formatted_text(data_node_children(node, header_opts), filter_block_hash(header_opts))
      end
    end
